...
public class AnimController : MonoBehaviour
{
    ...
    void Update()
    {
        if ((Input.GetButtonDown("left")|| Input.GetButtonDown("right")) && (mainCamera.transform.position == new Vector3(-0.59f,1.21f,-1.44f)))
        {
            GetComponent<Animator>().Play("ToMtHutt");
        }
        else if ((Input.GetButtonDown("right") || Input.GetButtonDown("right")) && (mainCamera.transform.position == new Vector3(-1.45f,0.9f, -0.61f)))
        {
            GetComponent<Animator>().Play("ToMtMurchison");
        }
        if(Input.GetButtonDown("ok") && (mainCamera.transform.position == new Vector3(-1.45f, 0.9f, -0.61f)))
        {
            gameState.inMtHutt = true;
            GetComponent<Animator>().Play("MtHuttScreenWipeEntry");
            Invoke(nameof(MtHutt), 2f);
        }
        else if(Input.GetButtonDown("ok") && (mainCamera.transform.position == new Vector3(-0.59f, 1.21f, -1.44f)))
        {
